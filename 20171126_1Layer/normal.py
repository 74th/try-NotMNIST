...
def learningNormal(datasets: Datasets, train_subset: int):
    ...
    with graph.as_default():
        ...
        # ウェイトは最初は乱数値
        # INPUT x OUTPUT の行列
        weight1 = tf.Variable(tf.truncated_normal([IMG_SIZE * IMG_SIZE, NUM_KIND_LABEL]))
        # バイアスはゼロ
        # OUTPUT のベクトル
        bias1 = tf.Variable(tf.zeros([NUM_KIND_LABEL]))
        # 隠れ層1
        # 活性関数はRELUを使うことにする
        hidden1 = tf.nn.relu(tf.matmul(dataset, weight1) + bias1)

        weight0 = tf.Variable(tf.zeros([NUM_KIND_LABEL, NUM_KIND_LABEL]))
        bias0 = tf.Variable(tf.zeros([NUM_KIND_LABEL]))
        logit = tf.nn.softmax(tf.matmul(hidden1, weight0) + bias0)
        ...
        # valid_prediction = tf.nn.softmax(tf.matmul(tf_valid_dataset, weight1) + bias1)
        # test_prediction = tf.nn.softmax(tf.matmul(tf_test_dataset, weight1) + bias1)
        ...
        for step in range(num_steps):
            ...
            if step % 100 == 0:
                ...
                print("%5d,%3.6f,%.6f,%.6f,%.6f" %
                      (step, c_loss, c_accurancy, valid_occuracy[0], 0))
