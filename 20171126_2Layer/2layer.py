...
def learningNormal(datasets: Datasets, train_subset: int):
    ...
    with graph.as_default():
        ...
        # 隠れ層2
        weight2 = tf.Variable(tf.truncated_normal([NUM_KIND_LABEL, NUM_KIND_LABEL]))
        bias2 = tf.Variable(tf.zeros([NUM_KIND_LABEL]))
        ...
        logit = tf.nn.softmax(tf.matmul(hidden2, weight0) + bias0)
